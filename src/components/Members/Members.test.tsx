...
describe('Member', () => {
  test('with empty member list does not render member data', () => {
    const { container } = render(<Members members={[]} />)
    const memberData = container.querySelector('.Members-data')
    expect(memberData).toBeNull()
  })

  test('with member list does render member data', () => {
    const { container } = render(<Members members={testData} />)
    const memberData = container.querySelector('.Members-data')
    expect(memberData).toBeInTheDocument()
  })
})
